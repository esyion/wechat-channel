...
| `src/manager/types.ts` | 50 | Multi-bot types: `BotManager`, `BotInfo`, `BotCredentials`, `BotCredentialStore`, `CreateBotManagerOpts` |
| `src/manager/sanitize.ts` | 17 | `sanitizeBotId()` — `[A-Za-z0-9_-]+` allowlist, blocks path traversal in `stateDir` |
| `src/manager/registry.ts` | 57 | `JsonBotCredentialStore` — default plaintext `bots.json` credential store |
| `src/manager/create.ts` | 81 | `createBotManager()` factory — wraps per-bot `createChannel`, tags callbacks with `botId` |
...
5. **TDD throughout.** Every `src/**/*.ts` module has a co-located `test/*.test.ts`. Tests run locally with `vitest`. Note: `*.test.ts` and `vitest.config.ts` are `.gitignore`d (see commit `060e5bf`, which removed all tests after CI OOM) — they are kept local only, are not committed, and never reach CI or the published tarball. `vitest` is a local-only devDependency.
...
npm test                   # vitest run (local only — see note below)
...
**Note on tests**: `*.test.ts` and `vitest.config.ts` are `.gitignore`d (commit `060e5bf` removed all tests after a CI OOM). They are kept local only — not committed, not run in CI, not included in the published package. `vitest` is a local-only devDependency. Run the suite locally with `npm test`.
